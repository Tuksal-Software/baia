...
<div class="container mx-auto px-4 py-8">
    ...
    <nav class="text-sm mb-6">
        <ol class="flex items-center gap-2 text-gray-600">
            <li><a href="{{ route('home') }}" class="hover:text-purple-600">Ana Sayfa</a></li>
            <li>/</li>
        ...
        <div>
            ...
            @if($product->reviews_count > 0)
                <div class="flex items-center gap-2 mb-4">
                    ...
                    <span class="text-gray-600">({{ $product->reviews_count }} degerlendirme)</span>
                </div>
            @endif

            <div class="mb-6">
                @if($product->is_on_sale)
                    <span class="text-3xl font-bold text-red-600">{{ number_format($product->sale_price, 2) }} TL</span>
                    <span class="text-xl text-gray-400 line-through ml-2">{{ number_format($product->price, 2) }} TL</span>
                    <span class="bg-red-100 text-red-600 text-sm px-2 py-1 rounded ml-2">%{{ $product->discount_percentage }} Indirim</span>
                @else
                    <span class="text-3xl font-bold text-gray-800">{{ number_format($product->price, 2) }} TL</span>
                @endif
            </div>
            ...
            @if($product->variants->count() > 0)
                <div class="mb-6" x-data="{ selectedVariant: null }">
                    <h3 class="font-semibold text-gray-800 mb-2">Varyant Secin:</h3>
                            ...
                            <label class="cursor-pointer">
                                ...
                                <span class="block px-4 py-2 border rounded-lg peer-checked:bg-purple-600 peer-checked:text-white peer-checked:border-purple-600 hover:border-purple-400">
                                    ...
                                    @if($variant->price_difference != 0)
                                        <span class="text-sm">({{ $variant->price_difference > 0 ? '+' : '' }}{{ number_format($variant->price_difference, 2) }} TL)</span>
                                    @endif
                                </span>
                            </label>
                        @endforeach
            ...
            @else
                <div class="bg-red-100 text-red-600 p-4 rounded-lg mb-6"><i class="fas fa-exclamation-circle mr-2"></i>Bu urun su anda stokta yok.</div>
            @endif

            <!-- Specifications -->
            @if($product->specifications->count() > 0)
                <div class="border-t pt-6">
                    <h3 class="font-semibold text-gray-800 mb-4">Urun Ozellikleri</h3>
            ...
            @if($product->features->count() > 0)
                <div class="border-t pt-6 mt-6">
                    <h3 class="font-semibold text-gray-800 mb-4">Ozellikler</h3>
                    <ul class="space-y-2">
    ...
    @if($product->description)
        <div class="mt-12 bg-white rounded-lg p-6">
            <h2 class="text-xl font-bold text-gray-800 mb-4">Urun Aciklamasi</h2>
    ...
    <!-- Reviews -->
    <div class="mt-12 bg-white rounded-lg p-6">
        <h2 class="text-xl font-bold text-gray-800 mb-6">Musteriler Yorumlar ({{ $product->reviews_count }})</h2>
                ...
                @foreach($product->approvedReviews as $review)
                    <div class="border-b pb-4">
                        <div class="flex items-center gap-2 mb-2">
                            ...
                            @if($review->is_verified)<span class="text-green-600 text-xs"><i class="fas fa-check-circle"></i> Dogrulanmis</span>@endif
                        </div>
        ...
        <!-- Review Form -->
        <div class="border-t pt-6">
            <h3 class="font-semibold text-gray-800 mb-4">Yorum Yap</h3>
            <form action="{{ route('reviews.store', $product) }}" method="POST" class="space-y-4">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <input type="text" name="customer_name" placeholder="Adiniz" required class="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500">
                    <input type="email" name="customer_email" placeholder="E-posta" required class="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500">
                </div>
                <div><label class="block text-sm text-gray-600 mb-1">Puaniniz</label><select name="rating" required class="border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500"><option value="5">5 Yildiz</option><option value="4">4 Yildiz</option><option value="3">3 Yildiz</option><option value="2">2 Yildiz</option><option value="1">1 Yildiz</option></select></div>
                <textarea name="comment" rows="4" placeholder="Yorumunuz (istege bagli)" class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500"></textarea>
                <button type="submit" class="bg-purple-600 text-white px-6 py-2 rounded-lg hover:bg-purple-700">Gonder</button>
            </form>
        </div>
    </div>
    ...
    @if($relatedProducts->count() > 0)
        <div class="mt-12">
            <h2 class="text-xl font-bold text-gray-800 mb-6">Benzer Urunler</h2>
