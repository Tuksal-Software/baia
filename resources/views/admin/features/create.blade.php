...
<div class="max-w-2xl">
    <div class="flex items-center gap-2 mb-6">
        ...
        <h2 class="text-xl font-semibold">Yeni Ozellik</h2>
    </div>

    <form action="{{ route('admin.features.store') }}" method="POST" class="bg-white rounded-lg p-6">
        @csrf
        <div class="space-y-4">
            <div>
                ...
                <select name="icon" required class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500">
                    @foreach($icons as $key => $label)
                        <option value="{{ $key }}" {{ old('icon') === $key ? 'selected' : '' }}>{{ $label }} ({{ $key }})</option>
                    @endforeach
                </select>
                <p class="text-xs text-gray-500 mt-1">Lucide ikonlari kullanilmaktadir.</p>
            </div>

            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Baslik *</label>
                <input type="text" name="title" value="{{ old('title') }}" required class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500">
            </div>
            ...
            <div>
                ...
                <input type="text" name="link" value="{{ old('link') }}" placeholder="/sayfa/..." class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500">
            </div>
            ...
            <div>
                ...
                <select name="position" required class="w-full border rounded-lg px-4 py-2 focus:ring-2 focus:ring-purple-500">
                    @foreach($positions as $key => $label)
                        <option value="{{ $key }}" {{ old('position', 'home') === $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
            </div>
